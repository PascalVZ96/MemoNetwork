-- MemoNetwork Alpha 10 Rank Manager
...
function MemoNetwork.Ranks.Get(ply)
    ...
    if ply.GetUserGroup then
        ...
        if group == "superadmin" or group == "owner" then
            return {name = "OWNER", color = Color(255, 145, 0), sort = 1}
        end

        if group == "admin" then
            return {name = "ADMIN", color = Color(80, 160, 255), sort = 2}
        end
    end
...
function MemoNetwork.Ranks.CanAdmin(ply)
    ...
    return MemoNetwork.Ranks.IsOwner(ply) or ply:IsAdmin()
end
